...
$\epsilon$ is chosen to increase exponentially within a suitable range depending
on the experiment (roughly between $1\%$ and $20\%$ of the respective
threshold level).
...
\begin{figure*}[t]
  ...
  \caption{Performance of explicit threshold sequential algorithms on the
           three datasets.
           (a), (b) \acl and \str are comparable and both clearly outperform
           \var.
           (c) \str performs poorly due to limited exploration.
           (d) An example of a \str execution getting stuck: even after $400$
           iterations it has only achieved an $F_1$-score of $0.37$.
           (e), (f) The results of (c) in more detail: About a third of \str's
           executions achieve an $F_1$-score of less than $0.4$, while \acl
           always achieves an $F_1$-score of at least $0.9$.
           }
  \label{fig:exp}
\end{figure*}
...
\figsref{fig:ping-seq}--\ref{fig:chl-seq} compare the performance of the
